...
public class RHW2Test {

	private String[] words = { "ant", "bee", "cat", "dog", "emu", "fox", "gem", "hat", "ice", "jar" };
	...
	// 5 points
	@Test
	public void test05PutDuplicateDelete() {
		RSequentialSearchST<String, Integer> st = new RSequentialSearchST<String, Integer>();
		for(int i = 0; i < 10; i++) {
			st.put(words[i], i);
		}
		...
		for(int i = 0; i < 10; i++) {
			assertNull(st.get(words[i]));
		}
	}

	// 5 points
	@Test
	public void test05PutDuplicates() {
		RSequentialSearchST<String, Integer> st = new RSequentialSearchST<String, Integer>();

		// Put 5 words in order into the ST.
		for (int i = 1; i < 5; i++) {
			st.put(words[i], i);
			assertEquals(i, st.size());
		}
